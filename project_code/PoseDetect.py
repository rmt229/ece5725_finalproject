...
class PoseDetect:
    ...
    def detect(self):
        while self.in_frame is not None:
            ...
            self.drop_pts = list(np.unique(np.where(coords ==0)[0]))
            ...
            self.keypoint_positions = coords * self.output_stride + offset_vectors

            

            # Loop over all detections and draw detection box if confidence is above minimum threshold
            for i in range(len(self.keypoint_positions)):
                #don't draw low confidence poi nts
                if i in self.drop_pts:
                    continue
                # Center coordinates
                x = int(self.keypoint_positions[i][1])
                y = int(self.keypoint_positions[i][0])
            ...
            self.detection_frame = self.draw_lines(self.keypoint_positions, frame_resized, self.drop_pts)
